...
class Weaver::InsertPosition
  ...
  def initialize(position, set, text)
    if position < 0
      raise Weaver::Error.new(position, set), 'Tried to create a negative InsertPosition'
    end
  ...
  def <=>(other)
    if self.position < other.position
      ...
      if self.set < other.set
        ...
        raise Weaver::Error.new(position, set), 'Overlap between same set of BodyPositions'
      end
    end
  end
end
